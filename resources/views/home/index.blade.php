...
<section class="container-fluid provider-sec-box">
    <div class="provider-sec container">
        <form action="{{ url('listing') }}" method="get" id="searchForm">
            @csrf
            <div class="inner">
                <label for="subcategories" class="sr-only">Select Service</label>
                <select class="form-control dropdown1 address" id="subcategories" name="services[]">
                    <option value="">I am looking for…</option>
                    @foreach($subcategories as $subcategory)
                        <option value="{{ $subcategory->id }}" data-name="{{ $subcategory->category->slug . '/' . $subcategory->slug }}">
                ...
                <div class="d-flex align-items-center location">
                    <label for="locations" class="sr-only">Select Location</label>
                    ...
                    <div class="greybox">
                        ...
                        <div class="d-lg-flex userbox">
                            <div class="d-lg-flex d-md-flex user-img">
                                ...
                                @endphp
                                <img src="{{ $avatarUrl }}" alt="Reviewer Avatar" class="img-fluid d-md-inline d-table mx-auto">
                        ...
                        <div class="col-md-12">
                            ...
                            <ul>
                                @foreach($category->subcategory->take(5) as $subcategory)
                                    <li>
                                        <a href="{{ url('companies/' . $category->slug . '/' . $subcategory->slug) }}">
                                            {{ $subcategory->subcategory }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="col-md-12 pl-2">
                            ...
                            <ul>
                                @foreach($subcategory->subcat_child->take(5) as $child)
                                    <li>
                                        <a href="{{ url('companies/' . $subcategory->category->slug . '/' . $subcategory->slug . '/' . $child->slug) }}">
                                            {{ $child->name }}
                                        </a>
                                    </li>
                                @endforeach
                    ...
                    <div class="col-md-12 moverscol1">
                        @foreach($modelReferences as $reference)
                            <div class="ptitle mb-2 mb-lg-0">
                                <button class="foreign-key-btn" data-key="{{ $reference['foreign_key_name'] }}">
...
<script>
    ...
    jQuery(function ($) {
        ...
        setAction = function () {
            ...
            const type = ($loc.data('type') || '').toString();
