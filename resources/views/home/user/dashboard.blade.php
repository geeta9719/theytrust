...
<section class="company_profile dash_profile pt-5 container">
    <div class="row dash-row dashsecond">
        <div class="col-md-3 brdright pt-2 pb-2">
            <h4>Profile</h4>
        </div>
        <div class="col-md-6 brdright pt-2 pb-2">
            <h4>Reviews</h4>
        </div>
    ...
    <div class="borderbox">
        <div class="row dash-row company text-center">
            <div
                class="col-md-3 brdright companybox leftbrd d-flex align-items-center justify-content-center secondrow">
                Last updated: {{ date('M d, Y', strtotime($company->updated_at)) }}
            </div>
            <div class="col-md-6 brdright companybox d-flex align-items-center justify-content-center secondrow">
                Add at least three reviews to your profile to increase visibility
            </div>
            <div class="col-md-3 companybox border-0 d-flex align-items-center justify-content-center secondrow">
                <span>{{ $company->review ?? '0' }} Reviews Added</span>
            </div>
        </div>

        {{-- <div class="row company-dec dash-third">
            <div class="col-md-3 sponsored companybox pt-3">
                <!-- Profile details -->
                <div class="text-center mb-3">
                    <img src='{{ url("storage/$company->logo") }}' alt="" class="img-fluid img-fluid-custom">
                </div>
                ...
                @if($company->website)
                <p><i class="fa fa-tag" aria-hidden="true"></i> {{ $company->website }}</p>
                @endif
                @if($company->email)
                ...
                <p>
                    <?php $uid = !empty($company->user_id) ? $company->user_id : auth()->user()->id; ?>
                    @if($company->profile_type)
                    <span class="btn btn-sm disabled" aria-disabled="true" tabindex="-1">
                        {{ ucfirst($company->profile_type) }} Profile
                    </span>
                    @endif
                    <a href="{{ route('user.allinfo', $uid) }}" class="btn btn-sm">Update Profile</a>
                </p>
            </div>
            <div class="col-md-6 companybox sponsored pt-3">
                <!-- Reviews details -->
                <p>The first review to be published is a sponsored profile review. The rest of the reviews normally take
                    about 1-2 weeks to be published.</p>
                <a href=class="btn btn-secondary btn-sm">Manage Reviews</a>
                <a href=class="btn btn-secondary btn-sm">Request A Review</a>
            </div>
        ...
        <div class="row company-dec dash-third">
            <div class="col-md-12 pt-3 sponsored sponsoredright">
                ...
                <div class="row hidden-content" id="service-areas-content">
                    @foreach($serviceLines as $serviceLine)
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">Category: {{ $serviceLine['category_name'] }}</h5>
                                <p class="card-text">Percentage: {{ $serviceLine['inputValue'] }}</p>
                                <p class="card-text">Subcategories:</p>
                                <ul>
                                    @foreach($serviceLine['subcategories'] as $subcategory)
                                    <li>
                                        {{ $subcategory['subcategory_name'] }} ({{ $subcategory['value'] }}%)
                                        <ul>
                                            @foreach($subcategory['skills'] as $skill)
                                            <li>
                                                {{ $skill['skill_name'] }}
                                                @if(count($skill['subskills']) > 0)
                                                <ul>
                                                    @foreach($skill['subskills'] as $subskill)
                                                    <li>{{ $subskill['subskill_name'] }}</li>
                                                    @endforeach
                                                </ul>
                                                @endif
                                            </li>
                ...
                <button class="btn btn-primary" id="show-more-service-areas">Show More</button>
            </div>
        </div>
            ...
            <div class="col-md-12 pt-3 sponsored sponsoredright">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">{{ $industry->industry->name }} {{ $industry->percent }}%</h5>
                            </div>
                        </div>
                    </div>
                    @endforeach
        ...
        <div class="row company-dec dash-third">
            <div class="col-md-12 pt-3 sponsored sponsoredright">
                <h4>Client Sizes</h4>
                <div class="row">
                    @foreach($clientSizes as $clientSize)
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">{{ $clientSize->client_size->name }} {{ $clientSize->percent }}%
                                </h5>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-12 pt-3 sponsored sponsoredright">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">{{ $address->autocomplete }}</h5>
                                <p class="card-text">{{ $address->city }}, {{ $address->country_iso2 }}</p>
            ...
            <div class="col-md-12 pt-3 sponsored sponsoredright">
                ...
                <div class="row">
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <button class="btn btn-primary btn-sm" id="add-photos">Add Photos</button>
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <button class="btn btn-primary btn-sm" id="add-videos">Add Videos</button>
                                <p>Only YouTube embed files allowed</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <button class="btn btn-primary btn-sm" id="add-white-papers">Add White Papers</button>
                                <p>Only pdf allowed</p>
                            </div>
                        </div>
                    </div>
                </div>
                <a href="{{ route('portfolio.create') }}" class="btn btn-primary">Create Portfolio Item</a>
                <a href="{{ route('portfolio.index', ['company' => $company->id]) }}" class="btn btn-primary">View
                    Portfolio Items</a>
                <!-- New button to view portfolio items -->
            </div>
        </div>
    </div>
</section>
